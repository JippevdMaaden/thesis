...
def getGreyhoundServer():
	return 'http://ec2-54-93-124-194.eu-central-1.compute.amazonaws.com:8080/'
...
class CameraCone:
	...
	def __init__(self, worldorigin, cameraorigin, cameratarget, fov):
		...
		lr = [self.radius, -self.radius, -self.azimutlength]
		ur = [self.radius, self.radius, -self.azimutlength]
		ll = [-self.radius, -self.radius, -self.azimutlength]
		ul = [-self.radius, self.radius, -self.azimutlength]
		...
		#not normalized
		temppoint1 = [point[0], point[1], -point[2], 1]
...
class GreyhoundConnection():
	...
	def get_pointcloud(self, param_schema):
	    self.param_schema = param_schema
	    self.param_str = ''
	    
	    for param in self.param_schema:
		self.param_str += '{}={}'.format(str(param), str(self.param_schema[param]))
		self.param_str += '&'
	    
	    self.param_str = self.param_str[:-1]
		
	    j = self.info()
	    data = self.read()
	    writeLASfile(data, self.filename)

	def info(self):
	    url = self.base_url + self.resource + '/info'
	    u = urlopen(url)
	    data = u.read()
	    j = json.loads(data)
	...
	def writeLASfile(self, data, filename):
	    try:
		minx = min(d['X'])
		miny = min(d['Y'])
		minz = min(d['Z'])
